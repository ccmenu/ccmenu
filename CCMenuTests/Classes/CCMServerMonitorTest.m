...
- (void)testResetsDisplayNameOnExistingInstanceWhenSettingUpFromDefaults
{
    NSArray *firstDefaults = @[ [CCMProject projectWithName:@"connectfour" inFeed:@"http://test/cctray.xml"] ];
    [[firstDefaults objectAtIndex:0] setDisplayName:@"Test Project"];
    OCMExpect([defaultsManagerMock projectList]).andReturn(firstDefaults);
    [monitor setupFromUserDefaults];
    ...
    XCTAssertEqualObjects(@"connectfour", [[[monitor projects] objectAtIndex:0] displayName], @"Should have reset display name.");
}
